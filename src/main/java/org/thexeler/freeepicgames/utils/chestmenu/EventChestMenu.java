...
public class EventChestMenu extends AbstractContainerMenu {
    ...
    protected EventChestMenu(MenuType<?> type, int containerId, Player player, Container container, int rows) {
        ...
        if (FreeEpicGamesConfigs.isEnabledChestMenuPostEventBus) {
            NeoForge.EVENT_BUS.post(new MenuEvent.CreateEvent(player, containerId, this));
        }
        ...
        if (FreeEpicGamesConfigs.isEnabledChestMenuPostEventBus) {
            NeoForge.EVENT_BUS.post(new MenuEvent.OpenedEvent(player, containerId, this));
        }
    }
    ...
    public void clicked(int slotId, int button, @NotNull ClickType clickType, @NotNull Player player) {
        ...
        if (FreeEpicGamesConfigs.isEnabledChestMenuPostEventBus) {
            isCanceled = NeoForge.EVENT_BUS.post(new MenuEvent.ClickEvent(player, slotId, clickType, this)).isCanceled();
        }
        if (!isCanceled) {
    ...
    public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
        ...
        boolean isCanceled = true;
        if (FreeEpicGamesConfigs.isEnabledChestMenuPostEventBus) {
            isCanceled = NeoForge.EVENT_BUS.post(new MenuEvent.ClickEvent(player, index, ClickType.QUICK_MOVE, this)).isCanceled();
        }
        if (!isCanceled) {
    ...
    public void removed(@NotNull Player player) {
        ...
        if (FreeEpicGamesConfigs.isEnabledChestMenuPostEventBus) {
            NeoForge.EVENT_BUS.post(new MenuEvent.CloseEvent(player, this.containerId, this));
        }
