...
public enum WithTargetLocationZSuggestionFactory implements SuggestionProvider.Factory<ForgeCommandActor> {
    INSTANCE;
    ...
    public @Nullable SuggestionProvider<ForgeCommandActor> create(@NotNull Type type, @NotNull AnnotationList annotations, @NotNull Lamp<ForgeCommandActor> lamp) {
        if (annotations.get(WithTargetLocationZ.class) == null) {
            return null;
        }
        return context -> {
            ...
            if (player != null) {
                ...
                if (!player.serverLevel().getBlockState(pos).isAir()) {
                    ret.add(String.valueOf(pos.getZ()));
                }
            }
            return ret;
        };
